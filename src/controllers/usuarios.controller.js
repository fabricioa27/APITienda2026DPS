    ...
    UsuarioModel.getAll((err, data) => {
        if (err)
            return response.error(res,
                'Ha ocurrido un error al obtener los usuarios.', 500, 'ERROR_INTERNO');
...
exports.findOne = (req, res) => {
    ...
    if (!id)
        return response.error(res,
            'El id debe ser un número entero positivo.', 400, 'PARAMETRO_INVALIDO');
    UsuarioModel.findById(id, (err, data) => {
        if (err) {
            if (err.kind === 'not_found')
                return response.error(res,
                    "Usuario con id ${id} no encontrado.", 404, 'RECURSO_NO_ENCONTRADO');
            return response.error(res,
                "Error al obtener el usuario con id ${id}.", 500, 'ERROR_INTERNO');
        }
...
exports.create = (req, res) => {
    if (!req.body)
        return response.error(res,
            'El cuerpo de la solicitud no puede estar vacío.', 400, 'DATOS_INVALIDOS');
    ...
    UsuarioModel.create(usuario, (err, data) => {
        if (err)
            return response.error(res,
                'Ha ocurrido un error al crear el usuario.', 500, 'ERROR_INTERNO');
        ...
        if (err) {
            if (err.kind === 'not_found')
                return response.error(res,
                    'Usuario con id ${id} no encontrado.', 404, 'RECURSO_NO_ENCONTRADO');
            return response.error(res,
                "Error al actualizar el usuario con id ${id}.", 500, 'ERROR_INTERNO');
        }
